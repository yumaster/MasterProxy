...
namespace MasterProxyWinform
{
    public partial class ClientMngr : Form
    {
        ...
        private const string SERVICE_PATH = "MasterProxyWinService.exe";
        ...
        public bool IsServiceMode
        {
            get
            {
                return WinServiceHelper.IsServiceExisted(Global.MPClientServiceName);
            }
        }
        ...
        public ClientMngr()
        {
            ...
            //右下角小图标
            notifyIconMPClient.Icon = Properties.Resources.servicestopped;
            ...
            this.notifyIconMPClient.Text = MPVersion.MasterProxyClientName;
        ...
        //    ClientUserCacheItem cacheItem = UserCacheManager.GetUserCacheFromEndpoint(endpoint, Router.MpClientCachePath);
        //    if (cacheItem != null)
        //    {//TODO 3 显示出用户名
        ...
        private void StartOrStop()
        {
            ...
            if (btnStart.Tag.ToString() == START_TAG_TEXT)
            {
                if (isService)
                {
                    using (ServiceController control = new ServiceController(Global.MPClientServiceName))
                    {
                    ...
                    StartClientRouter(config, (status, tunelStr) =>
                    {
                        btnStart.Invoke(new Action(
                            () =>
                            {
                                ...
                                {
                                    notifyIconMPClient.BalloonTipText = L("内网穿透已启动");
                                    ...
                                    {
                                        notifyIconMPClient.BalloonTipText += "\r\n" + tunnel.ToString();
                                        listBox1.Items[ii].Text = tunnel.Substring(tunnel.IndexOf(':') + 1).Trim();
                                        ii++;
                                    }
                                    notifyIconMPClient.ShowBalloonTip(5000);
                                    SetUIToRunning();
                                }
        ...
        private void SetUIToRunning()
        {
            ...
            notifyIconMPClient.Icon = Properties.Resources.servicerunning;
            btnStart.Enabled = true;
        }
        ...
        private void RefreshServiceBtnStatus()
        {
            ...
            btnStart.Invoke(new Action(() =>
            {
                ServiceController control = new ServiceController(Global.MPClientServiceName);
                ...
                {
                    string tip = $"内网穿透后台服务({Global.MPClientServiceName})已启动，详情请查看文件日志";
                    SetUIToRunning();
                    notifyIconMPClient.BalloonTipText = tip;
                    ShowLogInfo(tip);
                }
                else if (tskStopping.IsCompleted)
                {
                    SetUIToStop();
                    ShowLogInfo($"内网穿透后台服务({Global.MPClientServiceName})已激活，但处于关闭状态。");
                }
                else
                {
                    string tip = L("服务状态异常。");
        ...
        private void StartClientRouter(MPClientConfig config, Action<ClientStatus, List<string>> loaded)
        {
        ...
        private void SetConfig(Router clientRouter, MPClientConfig config)
        {
        ...
        private void ExitProgram()
        {
            ...
            if (MessageBox.Show(tip, "MasterProxy", MessageBoxButtons.OKCancel) == DialogResult.OK)
            {
                notifyIconMPClient.Dispose();
                Environment.Exit(0);
            }
        }
        ...
        private void 启动内网穿透ToolStripMenuItem_Paint(object sender, PaintEventArgs e)
        {
            ...
            {
                this.启动内网穿透ToolStripMenuItem.Image = global::MasterProxyWinform.Properties.Resources.base_checkmark_32;
            }
            else
            {
        ...
        {
            config = ConfigHelper.ReadAllConfig<MPClientConfig>(Program.CONFIG_FILE_PATH);
        ...
        private void RefreshLoginBtnState()
        {
            ...
            var userCacheItem = UserCacheManager.GetUserCacheFromEndpoint(GetEndPoint(), Router.MpClientCachePath);
        ...
        {
            if (WinServiceHelper.IsServiceExisted(Global.MPClientServiceName))
            {
                        ...
                        case 100:    //按下的是Ctrl+Shift+O   
                            {
                                ...
                                {
                                    this.Hide();
                                    this.notifyIconMPClient.Visible = false;
                                }
                                else
                                {
                                    this.Show();
                                    this.notifyIconMPClient.Visible = true;
                                }
                            }; break;
        ...
        private void btnTest_Click(object sender, EventArgs e)
        {
            ...
            Application.DoEvents();
            MPDispatcher clientDispatcher = new MPDispatcher($"{tbxProviderAddr.Text}:{tbxWebPort.Text}");
