...
namespace MasterProxy.Infrastructure.Extension
{
    public partial class HttpServer
    {
        #region HTTPServer

        public IMasterLogger Logger;
        ...
        public HttpServer(IMasterLogger logger, IDbOperator dbop, IServerContext serverContext, IWebController controllerInstance)
        {
            Logger = logger;
            Dbop = dbop;
            //第一次加载所有mime类型
            ...
            try
            {
                ...
                else  //url中没有小数点则是接口
                {
                    ...
                    try
                    {
                        ...
                        {
                            if (request.Cookies["MPTK"] == null)
                                throw new Exception("用户未登录。");
                            //TODO cookie，根据不同的用户角色分配权限。
                            var UserClaims = StringUtil.ConvertStringToTokenClaims(request.Cookies["MPTK"].Value);
