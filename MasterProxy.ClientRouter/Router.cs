...
namespace MasterProxy.Client
{
    public class NullLogger : IMasterLogger
    {
    ...
    public class Router
    {
        private static string mpClientCachePath = null;

        public static string MASTER_CLIENT_CACHE_FILE = "cli_cache_v3.cache";
        ...
        private MPDispatcher ClientDispatcher;
        ...
        internal MPClientConfig ClientConfig;
        internal LoginInfo CurrentLoginInfo;

        internal static IMasterLogger Logger = new NullLogger();   //inject
        internal static Guid TimeStamp; //时间戳，用来标识对象是否已经发生变化

        public static string MpClientCachePath
        {
            get
            {
                if (mpClientCachePath == null)
                {
                    //代表nsmartproxy.clientrouter.dll所在的路径
                    string assemblyFilePath = Assembly.GetExecutingAssembly().Location;
                    string assemblyDirPath = Path.GetDirectoryName(assemblyFilePath);
                    MpClientCachePath = assemblyDirPath + "\\" + MASTER_CLIENT_CACHE_FILE;
                    //MpClientCachePath = System.Environment.CurrentDirectory + "\\" + MASTER_CLIENT_CACHE_FILE;

                }

                return mpClientCachePath;
            }
            set => mpClientCachePath = value;
        }
        ...
        public Router(IMasterLogger logger) : this()
        {
            Logger = logger;
        }
        ...
        //{
        //    SetConfiguration(configstr.ToObject<MPClientConfig>());
        //    return this;
        //}
        public Router SetConfiguration(MPClientConfig config)//start之前一定要执行该方法，否则出错
        {
            ClientConfig = config;
            ClientDispatcher = new MPDispatcher($"{ClientConfig.ProviderAddress}:{ClientConfig.ProviderWebPort}");
            return this;
        }
        ...
        public async Task Start(bool AlwaysReconnect = false, Action<ClientModel> complete = null)
        {
            ...
            while (!oneLiveToken.IsCancellationRequested)
            {
                ...
                try
                {
                    var clientUserCacheItem = UserCacheManager.GetUserCacheFromEndpoint(GetProviderEndPoint(), MpClientCachePath);
                    //显式使用用户名密码登录
                    ...
                    else
                    {
                        ...
                        //File.WriteAllText(MpClientCachePath, arrangedToken);
                        UserCacheManager.UpdateUser(arrangedToken, "",
                            GetProviderEndPoint(), MpClientCachePath);
                    }
                }
        ...
        private async Task<ValueTuple<string, int>> Login()
        {
            ...
            if (result.State == 1)
            {
                ...
                Router.Logger.Debug($"服务端版本号：{data.Version},当前适配版本号{MPVersion.MasterProxyServerName}");
                clientId = int.Parse(data.Userid);
                //File.WriteAllText(MpClientCachePath, arrangedToken);
                var endPoint = ClientConfig.ProviderAddress + ":" + ClientConfig.ProviderWebPort;
                UserCacheManager.UpdateUser(arrangedToken, CurrentLoginInfo.UserName, endPoint, MpClientCachePath);
            }
            else
            {
