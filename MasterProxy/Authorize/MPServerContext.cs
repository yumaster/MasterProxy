...
namespace MasterProxy.Authorize
{
    /// <summary>
    /// 存放服务端的状态，以供其他组件共享
    /// </summary>
    public class MPServerContext : IServerContext
    {
        public MPClientCollection Clients;
        public Dictionary<int, MPAppGroup> PortAppMap;//“地址:端口”和app的映射关系
        public Dictionary<int, MPAppGroup> UDPPortAppMap;//UDP “地址:端口”和app的映射关系
        public MPServerConfig ServerConfig;
        ...
        public MPServerContext()
        {
            TokenCaches = new HashSet<string>();
            Clients = new MPClientCollection();
            PortAppMap = new Dictionary<int, MPAppGroup>();
            UDPPortAppMap = new Dictionary<int, MPAppGroup>();
        ...
        public void CloseAllSourceByClient(int clientId, bool addToBanlist = false, bool isForceClose = false)
        {
            ...
            {
                MPClient client = Clients[clientId];
                string msg = "";
                lock (locker)
                {
                    foreach (var appKV in client.AppMap)
                    {
                        ...
                        {
                            //TODO 3 关闭时会造成连锁反应
                            //TODO 3 并且当mpgroup里的元素全都被关闭时，才remove掉这个节点
                            //PortAppMap[port]
                            //PortAppMap[port].CloseByHost();
                            var mpAppGroup = appMap[port];
                            foreach (var (host, app) in mpAppGroup)
                            {
                            ...
                            if (mpAppGroup.IsAllClosed())
                            {
                                Server.Logger.Info($"端口{port}所有app退出，侦听终止");
                                if (mpAppGroup.Listener != null)
                                {
                                    //如果port内所有的app全都移除，才关闭listener
                                    mpAppGroup.Listener.Server.NoDelay = true;
                                    mpAppGroup.Listener.Server.Close();
                                    mpAppGroup.Listener.Stop();
                                }

                                if (mpAppGroup.UdpClient != null)
                                {
                                    mpAppGroup.UdpClient.Close();
                                }
