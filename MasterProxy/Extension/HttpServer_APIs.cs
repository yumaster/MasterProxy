...
using MasterProxy.Infrastructure.Interfaces;
using MasterProxy.Shared;
using MasterProxy.Data.DBEntites;

namespace MasterProxy.Extension
{
    /// <summary>
    /// 这里存放API
    /// </summary>
    partial class HttpServerApis : IWebController
    {
        ...
        private MPServerContext ServerContext;
        ...
        {
            ServerContext = (MPServerContext)serverContext;
            Dbop = dbOperator;
        ...
        {
            //return $"Server:{Global.MasterProxyServerName} Client:{Global.MasterProxyClientName}";
            return MPVersion.MasterProxyServerName;
        }
...
window.location.href='main.html';
        ...
        public LoginFormClientResult LoginFromClient(string username, string userpwd)
        {
            ...
            string output = $"{username}|{DateTime.Now.ToString("yyyy-MM-dd")}";
            string token = EncryptHelper.AES_Encrypt(output);
            return new LoginFormClientResult { Token = token, Version = MPVersion.MasterProxyServerName, Userid = user.userId };
        }
        #endregion
        ...
        [API]
        [Secure]
        public MPClientConfig GetServerClientConfig(string userId = null)
        {
            ...
            var config = Dbop.GetConfig(userId)?.ToObject<MPClientConfig>();
            return config;
        }
        ...
        {
            MPClientConfig mpClientConfig = null;
            ...
            else
            {
                try
                {
                    mpClientConfig = config.ToObject<MPClientConfig>();
                    mpClientConfig.UseServerControl = true;
                    //mpClientConfig.ProviderAddress = HttpContext.Request.Url.Host;
                    // mpClientConfig.ProviderWebPort = ServerContext.ServerConfig.WebAPIPort;
                    // mpClientConfig.ConfigPort = ServerContext.ServerConfig.ConfigPort;
                    // mpClientConfig.ReversePort = ServerContext.ServerConfig.ReversePort;
                }
                catch (Exception e)
                {
                ...
                Dbop.SetConfig(userName, mpClientConfig.ToJsonString());
            }
        ...
        private void AddAppJsonItem(StringBuilder json, int key, MPApp value)
        {
            json.Append("{ ");
