...
namespace MasterProxy
{
    ...
    public class ClientConnectionManager
    {
       ...
       // public event EventHandler<AppChangedEventArgs> AppTcpClientMapReverseConnected = delegate { };
        ...
        public ClientConnectionManager SetServerContext(MPServerContext serverContext)
        {
        ...
        {
            MPApp mpApp = null;
            mpApp = ServerContext.UDPPortAppMap[consumerPort].ActivateApp;

            TcpClient client = mpApp.TcpClientBlocks.Peek();
            if (client == null)
            {
        ...
        public async Task<TcpClient> GetClientForTcp(int consumerPort, string host = null)
        {
            ...
            { mpApp = ServerContext.PortAppMap[consumerPort].ActivateApp; }
            else
            {
                if (!ServerContext.PortAppMap[consumerPort].TryGetValue(host, out mpApp))
                {
            ...
            if (mpApp == null) throw new KeyNotFoundException($"无法找到{consumerPort}下的任何一个客户端app");

            //TODO ***需要处理服务端长时间不来请求的情况（无法建立隧道）
            //TODO 2这里的弹出比较麻烦了
            TcpClient client = await mpApp.PopClientAsync();
            if (client == null)
            {
            ...
            mpApp.ReverseClients.Add(client);
            return client;
        }
        ...
        public byte[] ArrangeConfigIds(byte[] appRequestBytes, byte[] consumerPortBytes, int highPriorityClientId)
        {
            ...
            lock (_lockObject2)
            {
                ...
                for (int i = 0; i < appCount; i++)
                {
                    ...
                    MPApp app = ServerContext.Clients[clientId].AppMap[arrangedAppid];
                    ...
                    if (protocol == Protocol.UDP)
                    {
                        ...
                        {
                            ServerContext.UDPPortAppMap[port] = new MPAppGroup();
                        }
                    ...
                    else
                    {
                        ...
                        {
                            ServerContext.PortAppMap[port] = new MPAppGroup();
                        }
