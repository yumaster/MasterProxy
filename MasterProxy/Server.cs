...
using MasterProxy.Extension;
using MasterProxy.Infrastructure;
using MasterProxy.Authorize;
using MasterProxy.Interfaces;
using MasterProxy.Shared;
using static MasterProxy.Server;
using MasterProxy.Database;
using MasterProxy.Infrastructure.Extension;
using MasterProxy.Data.DBEntites;
...
namespace MasterProxy
{
    ...
    public class Server
    {
        public const string USER_DB_PATH = "./master_user.db";
        public const string SECURE_KEY_FILE_PATH = "./master_sec_key";

        protected ClientConnectionManager ConnectionManager = null;
        protected IDbOperator DbOp;
        protected MPServerContext ServerContext;

        internal static IMasterLogger Logger; //inject

        public Server(IMasterLogger logger)
        {
            //initialize
            Logger = logger;
            ServerContext = new MPServerContext();
        }
        ...
        public Server SetConfiguration(MPServerConfig config)
        {
        ...
        private void ConnectionManager_AppAdded(object sender, AppChangedEventArgs e)
        {
            ...
            int port = 0;
            Dictionary<int, MPAppGroup> appMap = null;
        ...
        /// <returns></returns>
        async Task ListenConsumeAsync(int consumerPort, Dictionary<int, MPAppGroup> appMap)
        {
            //TODO 区分http请求
            ...
            try
            {
                var mpAppGrp = appMap[consumerPort];
                mpAppGrp.ActivateApp.CancelListenSource = cts;
                if (mpAppGrp.ProtocolInGroup == Protocol.UDP)  //UDP协议侦听
                {
                    var consumerUdpClient = new UdpClient(consumerPort);
                    mpAppGrp.UdpClient = consumerUdpClient;
                ...
                else  //TCP HTTP/HTTPS都走tcplistener侦听
                {
                    ...
                    mpAppGrp.Listener = consumerTcpListener;
            ...
            CancellationToken ct)
        {
            ...
            if (mpAppGroup.ProtocolInGroup == Protocol.UDP)
            {
                try
                {
                    ...
                    {
                        if (mpAppGroup.UdpTransmissionTask == null)//一个app只产生一个udp隧道（共用隧道）
                        {
                            //传回连接 (异步)
                            mpAppGroup.UdpTransmissionTask = OpenUdpTransmission(receiveResult.RemoteEndPoint, tunnelStream, mpAppGroup, ct);
                        }
                        //int x = 1;
                    }
                }
        ...
        private async Task OpenUdpTransmission(IPEndPoint FXXXconsumerEndPoint,//这个endpoint没有意义
            NetworkStream providerStream, MPAppGroup mpAppGrp, CancellationToken ct)
        {
            try
            {
                //byte[] buffer = new byte[Global.ServerTunnelBufferSize];
                //int bytesRead;
                var mpApp = mpAppGrp.ActivateApp;
                var udpClient = mpAppGrp.UdpClient;
        ...
        private async Task ProcessConsumeTcpRequestAsync(int consumerPort, TcpClient consumerClient, CancellationToken ct)
        {
            ...
            var mpAppGroup = ServerContext.PortAppMap[consumerPort];
            MPApp mpApp = null;
            ...
            try
            {
                ...
                if (mpAppGroup.ProtocolInGroup == Protocol.HTTP /*|| mpAppGroup.ProtocolInGroup == Protocol.HTTPS*/)
                {//不论是http协议还是https协议，有证书就加密
                    ...
                    if (mpAppGroup.ContainsKey(host))
                    {
                        mpAppGroup[host].Tunnels.Add(tunnel); //bug修改：建立隧道
                    }
                    else
                    {
                    ...
                    mpApp = mpAppGroup[host];
                }
                else if (mpAppGroup.ProtocolInGroup == Protocol.TCP) //TCP
                {
                    s2pClient = await ConnectionManager.GetClientForTcp(consumerPort);
                    mpAppGroup.ActivateApp.Tunnels.Add(tunnel);//bug修改：建立隧道
                    mpApp = mpAppGroup.ActivateApp;
                }
            }
            ...
            //TODO 如果MPApp中是http，则需要进一步分离，通过GetHTTPClient来分出对应的client以建立隧道
            //II.弹出先前已经准备好的socket
            ...
            {
                Logger.Debug($"client:{mpApp.ClientId} app:{mpApp.AppId}写入失败,尝试切断该客户端");
                ServerContext.CloseAllSourceByClient(mpApp.ClientId);
                return;
            }
            ...
            try
            {
                await OpenTcpTransmission(consumerStream, providerStream, mpApp, transfering.Token);
            }
            finally
            {
                consumerClient.Close();
        ...
        private async Task ProcessHeartbeatProtocol(TcpClient client)
        {
            ...
            if (ServerContext.Clients.ContainsKey(clientID))
            {
                ...
                var mpClient = ServerContext.Clients[clientID];
                mpClient.LastUpdateTime = DateTime.Now;

                //2.3 TODO 5 发送保活数据包,一旦建立了这种机制就不需要iocontrol的keepalive了
                try
                {
                    //Server.Logger.Debug($"尝试对{clientID}发送保活数据包..");//TODO 待删除
                    foreach (var kv in mpClient.AppMap)
                    {
        ...
        private async Task<bool> ProcessAppRequestProtocol(TcpClient client, bool IsReconnect = false)
        {
            ...
            ////4.给MPClient关联configclient
            //mpClient.LastUpdateTime
            ...
            CancellationToken ct)
        {
            try
            {
                Server.Logger.Debug($"New client ({mpApp.ClientId}-{mpApp.AppId}) connected");

                Task taskC2PLooping = ToStaticTransfer(ct, consumerStream, providerStream, mpApp);
                Task taskP2CLooping = StreamTransfer(ct, providerStream, consumerStream, mpApp);

                //任何一端传输中断或者故障，则关闭所有连接
                var comletedTask = await Task.WhenAny(taskC2PLooping, taskP2CLooping);
                Logger.Debug($"Transferring ({mpApp.ClientId}-{mpApp.AppId}) STOPPED");
        ...
        private async Task StreamTransfer(CancellationToken ct, Stream fromStream, Stream toStream, MPApp mpApp)
        {
            using (fromStream)
            {
                byte[] buffer = new byte[Global.ServerTunnelBufferSize];
        ...
        private async Task ToStaticTransfer(CancellationToken ct, Stream fromStream, Stream toStream, MPApp mpApp)
        {
            using (fromStream)
            {
                byte[] buffer = new byte[Global.ServerTunnelBufferSize];
